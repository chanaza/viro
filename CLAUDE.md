...
- **easy.co.il**: לקנות → כל החנויות → הקלד שם רשת (חובה) → בחר סניפים → Enter → radio button "טוב" → כל הארץ
- **maps.google.com**: חיפוש בתיבה → בחר "כל המיקומים" אם קיים
- **d.co.il**: חיפוש שם הרשת → לחץ "הצג עוד תוצאות" בלולאה → חלץ `li[data-card]`
...
## אתגרים פתוחים
- **Google Maps**: extract עובד בצורה לא עקבית — לפעמים מצליח (12 תוצאות), לפעמים הסוכן נכנס ללולאת evaluate כושלת. צריך להבין מתי extract עובד על Maps ומדוע לא תמיד.
- **easy.co.il**: הסוכן מבצע את הצעדים הנכונים אך לפעמים הדפדפן מתרסק בין הטאבים (Instagram → easy.co.il) וטאב easy.co.il נפתח ריק. בעיית תשתית של browser-use.
- **d.co.il**: כפתור "הצג עוד תוצאות" לא תמיד מזוהה — הסוכן נכשל בחילוץ גם כשיש 34 תוצאות. צריך לוודא שהכפתור קיים ושההוראה ברורה.
- **שלב post-processing**: ניקוי נתונים (הסרת מספרים סידוריים, כפילויות, עסקים לא-רלוונטיים) — לא מומש עדיין.
